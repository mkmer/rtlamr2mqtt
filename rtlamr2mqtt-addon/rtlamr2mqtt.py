...
def load_id_file(sdl_ids_file):
    ...
    with open(sdl_ids_file) as f:
        for line in f:
            li = line.strip()
            if (
                re.match(r"(^(0[xX])?[A-Fa-f0-9]+:(0[xX])?[A-Fa-f0-9]+$)", li)
                is not None
            ):
    ...
    for dev in usb.core.find(find_all=True):
        for known_dev in DEVICE_IDS:
            ...
            if dev.idVendor == int(usb_id, 16) and dev.idProduct == int(usb_vendor, 16):
                index += 1
                devices_found[known_dev] = {
                    "bus_address": "{:03d}:{:03d}".format(dev.bus, dev.address),
                    "index": index,
                }
                log_message(
                    "RTL SDR Device {} found on USB port {:03d}:{:03d} - Index: {}".format(
                        known_dev, dev.bus, dev.address, index
                    )
                )
                break
...
class MqttSender:
    def __init__(self, mqtt_config):
        ...
        cert_reqs = (
            ssl.CERT_NONE
            if mqtt_config.get("tls_insecure", True)
    ...
    def publish(self, **kwargs):
        ...
        will = {
            "topic": self.d["availability_topic"],
            "payload": "offline",
            "qos": 1,
            "retain": True,
        }
        try:
            publish.single(
                ...
                qos=qos,
                retain=retain,
                hostname=self.d["hostname"],
                port=self.d["port"],
                client_id=self.d["client_id"],
                keepalive=60,
                will=will,
...
def shutdown(signum, frame):
    ...
    if signum != 0 and frame != 0:
        ...
        if not running_in_listen_only_mode:
            if mqtt_sender:
                mqtt_sender.publish(
                    topic=availability_topic, payload="offline", retain=True
                )
...
def load_config(argv):
    ...
    if running_as_addon:
        if config["mqtt"].get("host", None) is None:
            ...
            log_message(
                "MQTT Host defined in config file. Ignoring Supervisor Configuration..."
            )
...
def send_ha_autodiscovery(meter, mqtt_config):
    ...
    discover_topic = "{}/sensor/rtlamr/{}/config".format(
        mqtt_config["ha_autodiscovery_topic"], meter["name"]
    )
    discover_payload = {
        "name": meter["name"],
    ...
    mqtt_sender.publish(
        topic=discover_topic, payload=dumps(discover_payload), qos=1, retain=True
    )
...
def tickle_rtl_tcp(remote_server):
    ...
    log_message(
        "server: {}, host: {}, port: {}".format(remote_server, remote_host, remote_port)
    )
    conn = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    conn.settimeout(5)  # 5 seconds
    send_cmd = lambda c, command, parameter: c.send(
        pack(">BI", int(command), int(parameter))
    )
    try:
...
def listen_mode():
    ...
    if running_as_addon:
        ...
        if (
            re.match("127\.0\.0\.|localhost", config["general"]["rtltcp_server"])
            is None
        ):
            external_rtl_tcp = True
            log_message(
                "Using an external RTL_TCP session at {}".format(
                    config["general"]["rtltcp_server"]
                )
            )
    ...
    while True:
        for amrline in rtlamr.stdout:
            ...
            if json_output is not None and running_as_addon:
                mqtt_sender.publish(
                    topic=debug_topic, payload=dumps(json_output), retain=False
                )
...
if __name__ == "__main__":
    ...
    if not external_rtl_tcp:
        ...
        if (
            re.match(
                r"(^(0[xX])?[A-Fa-f0-9]{4}:(0[xX])?[A-Fa-f0-9]{4}$)", usb_device_id
            )
            is not None
        ):
            ...
            log_message(
                "No USB device specified in the config file, using the first found."
            )
    ...
    for meter in config["meters"]:
        ...
        if meter_id in meters or meter_name in meter_names:
            log_message(
                "Error: Duplicate meter name ({}) or id ({}) found in config. Exiting.".format(
                    meter_name, meter_id
                )
            )
            sys.exit(1)
        ...
        meters[meter_id]["state_topic"] = "{}/{}/state".format(
            config["mqtt"]["base_topic"], meter_id
        )
        meters[meter_id]["attribute_topic"] = "{}/{}/attributes".format(
            config["mqtt"]["base_topic"], meter_id
        )
        meters[meter_id]["name"] = meter_name
        meters[meter_id]["unit_of_measurement"] = str(
            meter.get("unit_of_measurement", "")
        )
    ...
    rtlamr_cmd = [
        ...
        "-format=json",
        "-filterid={}".format(",".join(meters.keys())),
    ] + rtlamr_custom
    #################################################################
    ...
    while True:
        ...
        if not external_rtl_tcp and (
            "rtltcp" not in locals() or rtltcp.poll() is not None
        ):
        ...
        for amrline in rtlamr.stdout:
            ...
            if config["general"]["verbosity"] == "debug" and not is_an_error_message(
                amrline
            ):
            ...
            if (
                json_output and "Message" in json_output
            ):  # If it is a valid JSON and is not empty then...
                # Extract the Meter ID
                meter_id_key = list_intersection(
                    json_output["Message"], ["EndpointID", "ID", "ERTSerialNumber"]
                )
                meter_id = (
                    str(json_output["Message"][meter_id_key]).strip()
                    if meter_id_key
                    else None
                )

                # Extract the consumption
                consumption_key = list_intersection(
                    json_output["Message"], ["Consumption", "LastConsumptionCount"]
                )
                raw_reading = (
                    str(json_output["Message"][consumption_key]).strip()
                    if consumption_key
                    else None
                )
                ...
                if meter_id and raw_reading:
                    if meter_id in meters:
                        if (
                            "format" in meters[meter_id]
                        ):  # We have a "format" parameter, let's format the number!
                            formatted_reading = format_number(
                                raw_reading, meters[meter_id]["format"]
                            )
                        else:
                            formatted_reading = str(
                                raw_reading
                            )  # Nope, no formating, just the raw number

                        log_message(
                            'Meter "{}" - Consumption {}. Sending value to MQTT.'.format(
                                meter_id, formatted_reading
                            )
                        )
                        ...
                        mqtt_sender.publish(
                            topic=state_topic, payload=formatted_reading, retain=True
                        )
                        meter_readings[meter_id] += 1

            if (
                config["general"]["sleep_for"] > 0
            ):  # We have a sleep_for parameter. Let's go to sleep!
        ...
        shutdown(0, 0)
        log_message(
            "Sleeping for {} seconds, see you later...".format(
                config["general"]["sleep_for"]
            )
        )
